...
export default function SambutanSection({ kepalaName, kepalaImage, kepalaQuote }: SambutanSectionProps) {
  const ref = useRef<HTMLDivElement>(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  // Get initials for signature fallback/avatar
  const initials = kepalaName
    ? kepalaName
        .split(" ")
        .map((w) => w[0])
        .slice(0, 2)
        ...
        <div ref={ref} className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-6 leading-tight">
              Bersama Wujudkan{" "}
              <span className="gradient-text">Desa yang Maju</span> dan Sejahtera
            </h2>
